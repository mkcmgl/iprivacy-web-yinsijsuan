...
/* 导出分页组件 */
export function pagination(app: App) {
...
export function useTitle() {
	...
	nextTick(() => {
		...
		const routeList = ['/login', '/register', '/forgotPwd']
...
export const globalComponentSize = (): string => {
	const stores = useThemeConfig(pinia);
	const { themeConfig } = storeToRefs(stores);
	return Session.get('themeConfig')?.globalComponentSize || themeConfig.value?.globalComponentSize;
};
