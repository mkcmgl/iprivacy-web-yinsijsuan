 ...
 * @method useOrganizeDetail().setOrganizeDetail() 触发初始化企业信息 pinia
...
export async function initFrontEndControlRoutes() {
	...
	// 获取系统参数
	if (useUserInfo().userInfos.roles.length !== 0 || useUserInfo().userInfos.positions.length !== 0) {
		await useUserInfo(pinia).setSystemParams();
	} else {
		Session.remove('systemParams');
	}
	// 触发初始化企业信息
	await useOrganizeDetail(pinia).setOrganizeDetail();
	// 添加动态路由
	await setAddRoute();
	// 设置递归过滤有权限的路由到 pinia routesList 中（已处理成多级嵌套路由）及缓存多级嵌套数组处理后的一维数组
	await setFilterMenuAndCacheTagsViewRoutes();
	// 无登录权限时，添加判断
	if (useUserInfo().userInfos.roles.length <= 0 && useUserInfo().userInfos.positions.length <= 0) return Promise.resolve(true);
}
