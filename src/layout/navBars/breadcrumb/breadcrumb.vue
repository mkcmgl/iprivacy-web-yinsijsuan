			...
			<transition-group name="breadcrumb">
				<el-breadcrumb-item v-for="(v, k) in state.breadcrumbList" :key="!v.meta.tagsViewName ? v.meta.title : v.meta.tagsViewName">
					<span v-if="k === state.breadcrumbList.length - 1" class="layout-navbars-breadcrumb-span">
						...
						<div v-if="!v.meta.tagsViewName" class="layout-navbars-breadcrumb-link">{{ v.meta.title }}</div>
						<div class="layout-navbars-breadcrumb-link" v-else>{{ v.meta.tagsViewName }}</div>
					</span>
					<a v-else class="layout-navbars-breadcrumb-link" @click.prevent="onBreadcrumbClick(v)">
...
.layout-navbars-breadcrumb {
	...
	.layout-navbars-breadcrumb-icon {
		cursor: pointer;
		font-size: 18px;
		color: var(--next-color-breadcrumb-color);
		height: 100%;
		width: 40px;
		opacity: 0.8;
		&:hover {
	...
	.layout-navbars-breadcrumb-link {
		font-size: 20px;
		color: var(--next-color-breadcrumb-color);
	}
}
